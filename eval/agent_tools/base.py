...
Each tool call delegates to BenchPipeline, which SSHes to the remote ARM
instance and runs `eval/agent_tools/remote_runner.py <op>` with JSON args on
stdin, receiving JSON results on stdout. This keeps the agent loop (LLM calls)
local while compile/evaluate/disassemble execute on the real ARM hardware.
...
# Serializes concurrent add_traces() calls from parallel agent threads.
...
class AgentTools(ABC):
    ...
    def compile(self, code: str) -> dict:
        ...
        result = self._pipeline.compile(self._definition.name, solution, timeout=120)
    ...
    def disassemble(self, fn: Optional[str] = None) -> dict:
        ...
        result = self._pipeline.disassemble(
            lc["so_path"],
    ...
    def submit(self, explanation: str = "") -> dict:
        ...
        # Compile
        solution = self.make_solution(code)
        compile_result = self._pipeline.compile(self._definition.name, solution, timeout=120)
        ...
        eval_result = self._pipeline.evaluate(
            self._definition.name,
            so_path,
    ...
    def _run_remote_fire_forget(self, cmd: str) -> None:
        """Run a shell command on the remote without waiting for or checking the result."""
        try:
            self._pipeline.run_shell(cmd, timeout=10)
        except Exception:
            pass
