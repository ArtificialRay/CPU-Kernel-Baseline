...
def run_agentic_eval(
    ...
    *,
    ...
    Graviton instance via SSH through BenchPipeline. Iterates until the agent
    calls submit() or max_turns is reached (triggering auto-submit of the best
    ...
    Args:
        ...
        pipeline: BenchPipeline managing the instance pool (thread-sticky handle).
    ...
    tools = ToolsCls(pipeline, definition, trace_set, author, bench_cfg=bench_cfg)
    ...
    family = pipeline.instance_type.split(".")[0] if pipeline.instance_type else ""
    isa_desc = _AGENT_ISA_LABELS.get(family, pipeline.instance_type or "AArch64")
