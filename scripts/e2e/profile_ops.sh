...
if [ "$NP" -gt 0 ]; then ARGS=(-p "$NP" -n 0); KIND=pp; else ARGS=(-p 0 -n "$NG"); KIND=tg; fi
perf record -F 999 -g -o "$OUT/perf.data" -- "$B/bin/llama-bench" -m "$M" -t "$T" "${ARGS[@]}" -r 3 -o json > "$OUT/llama-bench.json" 2> "$OUT/llama-bench.err"
perf report -i "$OUT/perf.data" --no-children --sort symbol --stdio 2>/dev/null | grep -v '^#' | grep -v '^$' > "$OUT/symbols.txt"
