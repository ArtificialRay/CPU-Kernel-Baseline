...
async def run_tool_sequence(
    ...
    *,
    ...
    """Drive one full compile/evaluate/disassemble/submit sequence over an
    already-initialized ClientSession. Raises AssertionError on any check
    failure. Returns submit()'s result dict.
    """
    ...
    if code is None:
        ...
        ref = next(
            (r for r in resources.resources if r.name == "reference-scalar-kernel.cpp"), None,
        )
        assert ref is not None, "no reference-scalar-kernel.cpp resource found — pass --code-file"
        read_result = await session.read_resource(ref.uri)
        code = read_result.contents[0].text  # type: ignore[union-attr]

    r = await session.call_tool("compile", {"code": code})
    compile_result = _tool_result_dict(r)
    assert compile_result.get("status") == "OK", compile_result
    if verbose:
        print(f"  compile: {compile_result}")

    r = await session.call_tool("evaluate", {"measure": True})
    ...
    assert "v1.cpp" in names, sorted(names)
    v1 = next(res for res in resources.resources if res.name == "v1.cpp")
    read_result = await session.read_resource(v1.uri)
    v1_text = read_result.contents[0].text  # type: ignore[union-attr]
    assert v1_text == code, "v1.cpp resource content doesn't match the code that was compiled"
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            return await run_tool_sequence(
                session, starter_code=starter_code,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.transport == "stdio":
        result = asyncio.run(run_stdio_sequence(args.command, args.spawn_args, starter_code=starter_code))
    else:
        if not args.endpoint:
            p.error("--endpoint is required for --transport sse")
        result = asyncio.run(run_sse_sequence(args.endpoint, starter_code=starter_code))
