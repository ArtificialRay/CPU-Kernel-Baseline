...
exposed through your MCP tool list (you don't need to guess a prefix —
your tool list already has the real tool names). There is no separate
`submit` tool — `evaluate()` auto-persists the best-performing version the
moment it beats your previous best; see "Finish and report" below.
...
- You are running in an empty, throwaway working directory with no local
  repo checkout and no filesystem access worth relying on — everything
  that matters (reading the baseline kernel, compiling, evaluating,
  disassembling, reading MCP resources) happens through your MCP tools
  against the remote instance, not through local shell commands.
...
listing/reading MCP resources won't show you any of it until you've
compile()'d this definition yourself in this connection, so don't rely on
those to check. Instead, for every definition, before your first
`compile()`:

1. Call `check_progress({"definition": "<that definition's name>"})`.
2. If it returns `{"has_prior_progress": false}`, there's no prior work —
   proceed to §1 below as normal.
3. If it returns `{"has_prior_progress": true, "best_version": M,
   "best_metrics": {...}, "best_code": "..."}`, someone already made
   progress on this definition. Skip §1 entirely (do NOT compile
   reference-scalar). Instead, your first two calls must be, in order:
   a. `compile({"definition": "<that definition's name>", "code":
      <best_code>})`
   b. `evaluate({})` on the result
   Do this even though you already know `best_metrics`, because you need to re-establish the "best-so-far" kernel version in your current session.
...
2. Read it. It `#include`s a per-definition `.h` header —
   ...
   Don't try to read it directly — it will 404.
