...
// Lives in ncnn/framework/ alongside the headers it stubs.
...
// Weak hook so a test binary can supply a real Layer for specific LayerType
// indices. Default returns 0 → fall through to dummy Layer below. The
// baseline conv test wires LayerType::Convolution to a real Convolution_arm
// so Convolution_arm::forwardDilation_arm's sub-layer actually computes.
__attribute__((weak)) Layer* test_make_layer_cpu_hook(int /*index*/) { return 0; }
...
{
    if (Layer* l = test_make_layer_cpu_hook(index)) return l;
    if (index == LayerType::Padding) return new Padding();
    return new Layer();
}
